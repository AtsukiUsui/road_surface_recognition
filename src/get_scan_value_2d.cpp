...
int main(int argc, char **argv) {
  ...
  LaserScan_subscriber =
      n.subscribe<sensor_msgs::LaserScan>("/diag_scan", 10, get_scan_value);
...
void get_scan_value(const sensor_msgs::LaserScan::ConstPtr &scan) {
  ...
  double sensor_hight =
      scan->ranges[laser_sumNum / 2] * sin(sensor_pitch_deg * PI / 180);
  double d_R2MLine =
      scan->ranges[laser_sumNum / 2] * cos(sensor_pitch_deg * PI / 180);
  ...
  printf("d_R2MLine:%f  ",
         d_R2MLine); // distance between robot and measurement line
  printf("d_MLine:%f\n",
