...
Making_Envir_Cloud::Making_Envir_Cloud() {
  diag_scan_sub = nh.subscribe<sensor_msgs::LaserScan>(
...
void Making_Envir_Cloud::diagScanCallback(
  ...
  if (listener.waitForTransform(
          ...
          ros::Duration(1.0), ros::Duration(0.01), &error_msg) == 0) {
  ...
  try {
    projector.transformLaserScanToPointCloud("/map", *scan_in, cloud_in,
                                             listener);
  ...
  std::string scan_file_path =
      "/home/atsuki/lab_ws/data/pcd/making_envir_cloud/laserscan_data/";
  ...
  std::string raw_file_path =
      "/home/atsuki/lab_ws/data/pcd/making_envir_cloud/raw_data/";
  ...
  if (flag == true) {
    ...
    // If Calculated inclination is over threshold value, process is stopped
    // because got scan values include obstacle.
    if (a <= -0.025 || a >= 0.025)
      return;
  }
  ...
  for (int i = 0; i < filtered_cloud->points.size(); i++) {
    ...
    if (flag == true) {
      if (filtered_cloud->points[i].z >=
          a * filtered_cloud->points[i].y + b + 0.038) {
  ...
  std::string filtered_file_path =
      "/home/atsuki/lab_ws/data/pcd/making_envir_cloud/filtered_data/";
  ...
  loop_rate.sleep();
  gettimeofday(&e, NULL);
  std::cout << "Compute time: " << std::setw(9)
            << (e.tv_sec - s.tv_sec) + (e.tv_usec - s.tv_usec) * 1.0E-6 << "s"
            << std::endl;
}
